...
export default function ROICDR3Length({ report, section_name, facetRowValues, facetColValues, setAppStatus }) {
  ...
  const reportCDR3 = report[section_name];
  const reportName = reportCDR3['name'];
  const reportTitle = reportCDR3['title'];
  const reportType = reportCDR3['report_type'];
  const reportXLabel = reportCDR3['x_label'];
  const reportYLabel = reportCDR3['y_label'];
  const reportFacetData = reportCDR3['data']; // list, one item per facet
  ...
  for (const facetData of reportFacetData) {
    ...
    const cdr3Length = facetData['group_values'];        // e.g. [2, 3, 4]
    const redundancy = facetData['measure_values'];         // e.g. [10, 20, 30]
    const redundancyPct = facetData['measure_pcts'];   // e.g. [20.0, 40.0, 40.0]

    // Check we have everything
    if (!(facetRowValue && facetColValue && cdr3Length && redundancy && redundancyPct)) {
      setAppStatus(
